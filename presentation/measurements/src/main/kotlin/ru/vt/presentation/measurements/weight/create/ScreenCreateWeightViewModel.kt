...
import ru.vt.domain.measurement.usecase.weight.AddWeightUseCase
import ru.vt.presentation.measurements.R
import java.util.*
import kotlin.random.Random

@OptIn(DependencyAccessor::class)
internal class ScreenCreateWeightViewModel(
    savedStateHandle: SavedStateHandle,
    eventHandler: EventHandler?,
    resourceManger: ResourceManger,
    navigator: Navigator,
    private val repository: WeightRepository
) : BaseViewModel<ScreenCreateWeightViewModel.State, ScreenCreateWeightViewModel.Action>(
    savedStateHandle = savedStateHandle,
    eventHandler = eventHandler,
    resourceManger = resourceManger,
    navigator = navigator
    ...
    private val useCase: AddWeightUseCase by lazyNone {
        AddWeightUseCase(repository)
    }
    ...
    override suspend fun processAction(action: Action) {
        when (action) {
            ...
            is Action.SetWeight -> setWeight(action.value)
            Action.Add -> add()
    ...
    private fun add() {
        val params: AddWeightUseCase.Params = AddWeightUseCase.Params(
            profileId = state.profileId,
            timestamp = state.date.atZone(ZoneId.systemDefault()).toInstant()
                .toEpochMilli() + Random.nextLong(5000),
            weightInGrams = state.weightInGrams
        )
    ...
    sealed class Action {
        ...
        data class SetWeight(val value: Int?) : Action()
    ...
    class Factory : ViewModelFactory<ScreenCreateWeightViewModel> {
        override fun create(handle: SavedStateHandle): ScreenCreateWeightViewModel =
            ScreenCreateWeightViewModel(
